const returnCanvasIfTheyExist = (
  ...
  selectors.forEach((selector: string) => {
    // const doc = document
    //   .querySelector("happy-place")
    //   .shadowRoot.querySelector("theme-provider")
    //   .shadowRoot.querySelector("happy-place-main-pane").shadowRoot;
    const element = parent.querySelector(selector) as HTMLCanvasElement;
...
const removeCanvasElementsFromDOM = (
...
const setUpEnlargedCanvasDimensions = (
  ...
  canvasElement.forEach((canvasElement: HTMLCanvasElement) => {
    ...
    canvasElement.height =
      (parentElement?.clientHeight ?? 1) * devicePixelRatio;
...
const scaleCanvasToOriginalSize = (
  canvasElement: HTMLCanvasElement[],
  parentElement: HTMLElement
) => {
  canvasElement.forEach((canvasElement: HTMLCanvasElement) => {
...
const appendCanvasElementsToDOM = (
...
export const setUpCanvas = (
  selectors: Array<string>,
  parentElementSelector: any
): HTMLCanvasElement[] | null => {
  try {
    const parentElement = parentElementSelector;
    const canvasElements = returnCanvasIfTheyExist(parentElement, selectors);
